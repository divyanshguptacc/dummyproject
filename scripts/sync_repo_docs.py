...
- AGENTS.md (structure routing section)
"""
...
# FIX #13: Python import path fix for detect_repo_structure
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'scripts'))
sys.path.insert(0, os.path.dirname(__file__))

# FIX #22: Handle missing detect_repo_structure.py gracefully
try:
    from detect_repo_structure import detect_project_type
except ImportError:
    def detect_project_type(root=None):
        """Fallback when detect_repo_structure.py is not available."""
        return {'type': 'UNKNOWN', 'paths': {}, 'stack': {}, 'confidence': 0.0}
...
AUTO_STRUCTURE_START = "<!-- verity:agents-structure:start -->"
AUTO_STRUCTURE_END = "<!-- verity:agents-structure:end -->"
...
def detect_project_structure(config: dict[str, Any]) -> dict[str, Any]:
    """Get project structure from config or auto-detect."""
    # First check if structure is defined in config
    project = config.get("project", {})
    if isinstance(project, dict) and project.get("type"):
        return project
...
def render_agents_structure_block(structure: dict[str, Any]) -> str:
    """Render the structure section for AGENTS.md."""
    ...
    # Add path info with routing guidance
    if paths:
        lines.append("**Code Locations**:")
        if paths.get("frontend"):
            lines.append(f"- Frontend code: `{', '.join(paths['frontend'])}`")
        if paths.get("backend"):
            lines.append(f"- Backend code: `{', '.join(paths['backend'])}`")
        if paths.get("shared"):
            lines.append(f"- Shared code: `{', '.join(paths['shared'])}`")
        lines.append("")
    
    # Add routing guidance based on project type
    lines.append("**Routing Guidance**:")
    if project_type == "MONO":
        lines.append("- This is a monorepo containing both frontend and backend code.")
        lines.append("- UI/styling changes → look in frontend paths")
        lines.append("- API/data changes → look in backend paths")
        lines.append("- Full-stack features may require changes in both")
    elif project_type == "FRONTEND":
        lines.append("- This is a frontend-only repository.")
        lines.append("- All UI, styling, and client-side logic changes go here.")
    elif project_type == "BACKEND":
        lines.append("- This is a backend-only repository.")
        lines.append("- All API, data, and server-side logic changes go here.")
    else:
        lines.append("- Project type not detected. Review paths manually.")
...
def main() -> int:
    ...
    # Detect project structure for AGENTS.md
    structure = detect_project_structure(config)
    ...
    # Update AGENTS.md with structure section if it exists
    agents_content = None
    if agents_path.exists():
        agents_content = agents_path.read_text(encoding="utf-8")
        structure_block = render_agents_structure_block(structure)
        agents_content = apply_marker_block(
            agents_content,
    ...
    if agents_content is not None and write_if_changed(agents_path, agents_content):
